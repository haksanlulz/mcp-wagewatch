...
- Tests: 1327 lines, 92 tests in 2 files. `find ./test -name '*.test.ts' | xargs wc -l` for the lines; the test count is vitest's. The grep `grep -cE '^\s*(it|test)\(' test/*.test.ts` reads 87, because the per-tool unknown-argument cases are generated in a loop — one `it(` for six tests.
