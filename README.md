...
- **Name search: the endpoint's `LIKE` is case-SENSITIVE, and WHISARD stores names mixed-case.** Confirmed live 2026-09-14 by the pair that disproves the opposite claim this line used to make: `{"field":"trade_nm","operator":"like","value":"%KEVIN MISCH%"}` answers HTTP 204 with zero rows, while `"%Kevin Misch%"` answers HTTP 200 with case_id 1476714 and case_id 1419247. Of the 500 most recent rows by `findings_end_date`, 485 (97%) carry a mixed-case `trade_nm`. So the server searches `trade_nm` and `legal_name` for **three** case variants of the term — as typed, uppercased, and title-cased — as one `or` filter, wrapping each as `%term%`.
  ...
  On the wildcards: `_` is live-confirmed as a single-character wildcard (`%Kevin_Misch%` returns both Kevin Misch rows, the `_` matching the space), so an unescaped underscore in a caller's term would widen the search. The server escapes `%`, `_` and `\` after the case fold, using SQL's conventional backslash escape. ⚠️ **DOL's engine does NOT honour that backslash — settled live 2026-09-14, and it means the escaping is worse than doing nothing.** `{trade_nm like "%Kevin Misc\h%"}` answers HTTP 204 while the control `"%Kevin Misch%"` answers 200 with both rows in the same minute, so `\h` is matched literally rather than read as an escaped `h`. Consequence: a term containing `%`, `_` or `\` is sent as a pattern hunting a literal backslash and matches nothing — a confident "no cases found" on this dataset. The repair is a design question rather than a deletion (LIKE has no other way to make `%` literal, and DOL exposes no `ESCAPE` clause), so it is not in this change; the offline suite still asserts only that the REQUEST carries the escapes. See GAUNTLET known gaps.
