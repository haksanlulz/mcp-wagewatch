...
Everything has been run live against the real API with a real key: `npm run smoke`, **9 passed / 0 failed / 0 upstream / 0 skipped, 2026-09-14**. Six contract facts were only discoverable live, all handled and regression-tested:
...
⚠️ **That smoke run predates the 2026-09-14 fix-round-3 changes and has not been reproduced since.** The six-probe `LIKE` escape experiment above ran live that evening and DOL's rate limiter then answered `429` to every request for the following half hour, including four attempted smoke runs (`0 failed`, but `7 upstream` and exit **2**, which is a statement about DOL and not a pass). Two of this round's changes alter what goes on the wire — the search term is no longer escaped, and `back_wages_summary` now sends `sort_by=bw_atp_amt&sort=desc` — so **the live rung is outstanding for this round; re-run `npm run smoke` before treating the line above as current.** The request grammar itself is not new to DOL (`employer_violations` has always sent that same filter with that same sort, and passes live), but that is an argument, not a measurement.
...
| restoring the backslash escaping in `escapeLike` (R1) | both unescaped-term cases, the name filter's and `top_cases`' NAICS one, and nothing else |
