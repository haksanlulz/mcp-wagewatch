...
⚠️ **Top of the list, kept out of the numbering because the numbers below are cited elsewhere.** Whether DOL's `LIKE` honours `\` as an escape character is UNVERIFIED, and `escapeLike` depends on it.** `_` is live-confirmed as a single-character wildcard (2026-09-14: `%Kevin_Misch%` returns both Kevin Misch rows, the `_` matching the space), so escaping matters — but every probe of the escape itself, including a decisive one putting a backslash before a plain letter, answered HTTP 429 across several minutes of backoff. **The 429 is not about the character**: an alternating control/backslash run, three rounds, 429d on both arms — including a plain `%Kevin Misch%` that had answered 200 with two rows twenty minutes earlier. So the question is simply unanswered, not blocked by a filter, and a session with fresh quota can settle it with one request: `{trade_nm like "%Kevin Misc\h%"}` returns the two Kevin Misch rows if the backslash escapes and nothing if it is literal. It fails in both directions: if the backslash is inert, an escaped term becomes a pattern hunting a literal backslash and matches nothing, which on this dataset is a confident "no cases found". No rung covers it; the offline suite asserts only that the REQUEST carries the escapes.
